...
def run_viewer(verts, normals, uvs, indices, colour: np.ndarray,
    ...
    ground_tex_img_src = None
    ground_texture_path = os.path.join("textures", "pale-gray-pebble-floor.jpg")
    if os.path.isfile(ground_texture_path):
        try:
            ground_tex_img_src = Image.open(ground_texture_path).convert("RGB")
        except Exception as e:
            print(f"[viewer] Ground texture load failed ({ground_texture_path}): {e}")

    # ── camera state ───────────────────────────────────────────────────────
    ground_plane_z = float(np.min(verts[:, 2])) if len(verts) > 0 else 0.0
    eye_height = 1.0
    start_dist = max(4.0, float(col_height) * 1.6) if col_height > 0 else 6.0
    cam = {
        "x": 0.0,
        "y": -start_dist,
        "z": ground_plane_z + eye_height,
        "yaw": 0.0,
        "pitch": 8.0,
        "init_x": 0.0,
        "init_y": -start_dist,
        "init_z": ground_plane_z + eye_height,
        "init_yaw": 0.0,
        "init_pitch": 8.0,
    }
    movement = {
        "speed": max(2.0, float(col_height) * 0.35) if col_height > 0 else 3.0,
        "fly_speed": max(2.0, float(col_height) * 0.45) if col_height > 0 else 3.5,
        "look_sens": 0.14,
        "ground_z": ground_plane_z + eye_height,
        "flying": False,
        "space_last_t": -10.0,
        "space_double_t": 0.28,
        "space_drop_latch": False,
        "mouse_captured": True,
    }
    scene_extent = max(8.0, float(col_height) * 4.0) if col_height > 0 else 12.0

    config = pyglet.gl.Config(double_buffer=True, depth_size=24, samples=4)
    try:
        window = pyglet.window.Window(
            width=1280, height=720,
            caption="Joshua Roll Column Simulator (Walkable)  |  WASD move  Space=fly  double-Space=drop  mouse-look  Tab=toggle mouse  E=export  R=reset  Q=quit",
    ...
    gl_state = {"ready": False, "tex_id_main": None, "tex_id_lip": None, "tex_id_statue": None, "tex_id_ground": None,
    ...
    def gl_init():
        ...
        tex_id_ground = None
        if ground_tex_img_src is not None:
            gimg = ground_tex_img_src
            gw, gh = gimg.size
            if gw > max_tex_size or gh > max_tex_size:
                scale = min(max_tex_size / float(gw), max_tex_size / float(gh))
                nw = max(1, int(round(gw * scale)))
                nh = max(1, int(round(gh * scale)))
                gimg = gimg.resize((nw, nh), Image.LANCZOS)
                gw, gh = nw, nh
            gdata = np.array(gimg, dtype=np.uint8).ravel()
            tex_id_ground = GLuint(0)
            glGenTextures(1, ctypes.byref(tex_id_ground))
            glBindTexture(GL_TEXTURE_2D, tex_id_ground)
            glPixelStorei(GL_UNPACK_ALIGNMENT, 1)
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR)
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR)
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT)
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT)
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, gw, gh, 0,
                         GL_RGB, GL_UNSIGNED_BYTE,
                         gdata.ctypes.data_as(ctypes.c_void_p))
            try:
                glGenerateMipmap(GL_TEXTURE_2D)
            except Exception:
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR)
        ...
        gl_state["tex_id_ground"] = tex_id_ground
    ...
    def reset_camera():
        ...
        movement["space_drop_latch"] = False
    ...
    def on_draw():
        ...
        glLoadIdentity()
        # Align camera forward with world +Y for walk mode, then apply yaw/pitch.
        ...
        # ── texture ───────────────────────────────────────────────────────
        if gl_state["tex_id_ground"] is not None:
            ground_tile = max(2.0, scene_extent / 2.5)
            glEnable(GL_TEXTURE_2D)
            glBindTexture(GL_TEXTURE_2D, gl_state["tex_id_ground"])
            glColor4f(1.0, 1.0, 1.0, 1.0)
        else:
            ground_tile = 1.0
            glDisable(GL_TEXTURE_2D)
            glColor4f(0.32, 0.32, 0.30, 1.0)
        glBegin(GL_QUADS)
        glNormal3f(0.0, 0.0, 1.0)
        glTexCoord2f(0.0, 0.0)
        glVertex3f(-scene_extent, -scene_extent, ground_plane_z)
        glTexCoord2f(ground_tile, 0.0)
        glVertex3f(scene_extent, -scene_extent, ground_plane_z)
        glTexCoord2f(ground_tile, ground_tile)
        glVertex3f(scene_extent, scene_extent, ground_plane_z)
        glTexCoord2f(0.0, ground_tile)
        glVertex3f(-scene_extent, scene_extent, ground_plane_z)
        glEnd()
        glColor4f(1.0, 1.0, 1.0, 1.0)
    ...
    try:
        ...
        movement["mouse_captured"] = False

    @window.event
    def on_mouse_motion(x, y, dx, dy):
        if not movement["mouse_captured"]:
            return
        cam["yaw"] -= dx * movement["look_sens"]
        cam["pitch"] += dy * movement["look_sens"]
        cam["pitch"] = max(-89.0, min(89.0, cam["pitch"]))

    @window.event
    def on_mouse_drag(x, y, dx, dy, buttons, modifiers):
        if movement["mouse_captured"]:
            return
        cam["yaw"] -= dx * movement["look_sens"]
        cam["pitch"] += dy * movement["look_sens"]
        cam["pitch"] = max(-89.0, min(89.0, cam["pitch"]))
    ...
    def on_key_press(symbol, modifiers):
        ...
        if symbol in (key.Q, key.ESCAPE):
            ...
            movement["mouse_captured"] = not movement["mouse_captured"]
            try:
                window.set_exclusive_mouse(movement["mouse_captured"])
            except Exception:
                pass
        elif symbol == key.SPACE:
            t_now = time.perf_counter()
            if (t_now - movement["space_last_t"]) <= movement["space_double_t"]:
                cam["z"] = movement["ground_z"]
                movement["flying"] = False
                movement["space_drop_latch"] = True
            movement["space_last_t"] = t_now
        elif symbol == key.E:
            if export_fmt == "obj":
    ...
    print("[viewer] Opening 3D walkable viewer ...  (WASD move  Space fly  double-Space drop  Tab toggle mouse)")
    ...
    while not window.has_exit:
        ...
        dt = min(max(now_t - prev_t, 0.0), 0.05)
        prev_t = now_t

        yaw_rad = math.radians(cam["yaw"])
        # Match movement basis to the view transform (Rx(-90) then Rz(-yaw)).
        ...
        if move_len > 1e-8:
            inv = 1.0 / move_len
            cam["x"] += move_x * inv * movement["speed"] * dt
            cam["y"] += move_y * inv * movement["speed"] * dt

        if key_state[key.SPACE] and (not movement["space_drop_latch"]):
            cam["z"] += movement["fly_speed"] * dt
            movement["flying"] = True
        elif not movement["flying"]:
            cam["z"] = movement["ground_z"]

        if (not movement["flying"]) and cam["z"] < movement["ground_z"]:
            cam["z"] = movement["ground_z"]
