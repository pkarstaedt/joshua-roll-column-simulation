...
BASE_TOP_STEP_RUN = 0.1
# Height of each top step (world units).
BASE_TOP_STEP_HEIGHT = 0.2
...
ENDCAP_RADIUS = None   # None = COLUMN_RADIUS
...
ADD_TOP_STATUE = True
TOP_STATUE_GLB = "textures/capital.glb"
# Statue height as a multiple of CAPITAL_HEIGHT.
TOP_STATUE_HEIGHT_MULT = 1.3
...
# Clip wrapped helix triangles at the base plane (Z >= 0).
CLIP_HELIX_AT_BASE = True
HELIX_BOTTOM_CLIP_EPS = 1e-4
...
LIP_TEXTURE_STRIP_FRACTION = 0.02
...
MOLDING_RADIUS_OFFSET = 0.02
# Torus tube radius (thickness of molding profile).
MOLDING_TUBE_RADIUS = 0.05
# Torus tessellation.
...
def build_mesh(colour: np.ndarray, depth: np.ndarray,
    ...
    if wrap_column:
        ...
        # Hard-trim helix triangles at top and bottom planes.
        if len(indices) > 0:
            keep = np.ones(len(indices), dtype=bool)
            removed_top = 0
            removed_bottom = 0

            if CLIP_HELIX_AT_ENDCAP:
                z_tri_max = np.max(verts[indices][:, :, 2], axis=1)
                keep_top = z_tri_max <= (col_height + float(HELIX_TOP_CLIP_EPS))
                removed_top = int(np.count_nonzero(~keep_top))
                keep &= keep_top

            if CLIP_HELIX_AT_BASE:
                z_tri_min = np.min(verts[indices][:, :, 2], axis=1)
                keep_bottom = z_tri_min >= (0.0 - float(HELIX_BOTTOM_CLIP_EPS))
                removed_bottom = int(np.count_nonzero(~keep_bottom))
                keep &= keep_bottom
            ...
            if not np.all(keep):
                ...
                if removed_top > 0:
                    print(f"[clip] Removed {removed_top} triangles above endcap plane")
                if removed_bottom > 0:
                    print(f"[clip] Removed {removed_bottom} triangles below base plane")
    else:
        verts   = flat_verts
...
def main():
    ...
    print(f"  bottom_clip     : {CLIP_HELIX_AT_BASE}  (eps={HELIX_BOTTOM_CLIP_EPS})")
